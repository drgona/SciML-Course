...
def _(mo):
    mo.md(r"""
    # Neural ODE Training with Backpropagation Through Time (JAX)

    This notebook converts `week_4/scripts/jax/NODE_BPTT_vanilla.py` and
    `week_4/scripts/jax/NODE_BPTT_tuned.py` into one interactive app.

    **Design goal:** default controls recover the vanilla setup; optional controls let you reproduce tuned behavior.
...
def _(mo):
    ...
    Learned NODE and RK4 discretization:

    $$\dot{x}=f_\theta(x), \qquad x_{k+1}=\Phi_\theta(x_k,\Delta t).$$
    ...
    BPTT computes gradients through all integration steps:
    ...
    JAX evaluates these derivatives with composable `jax.value_and_grad` and optional `jax.jit`.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### 1.1 Code: NODE Parameterization

    We use a two-hidden-layer MLP:

    $$f_\theta(x)=W_3\,\rho\!\left(W_2\,\rho(W_1x+b_1)+b_2\right)+b_3,$$

    with ReLU activation $\rho$ and selectable initialization.
    """)
    return
...
def _(jax, jnp):
    ...
    def vanilla_init_layer(key, in_dim, out_dim):
        ...
        w = jax.random.uniform(key, (in_dim, out_dim), minval=-lim, maxval=lim)
        b = jnp.zeros((out_dim,), dtype=w.dtype)
        return {"w": w, "b": b}
    ...
    return init_mlp, init_mlp_vanilla, mlp_apply


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ### 1.2 Code: Optimizer and Learning-Rate Control
    ...
    plus optional weight decay, gradient clipping, and ReduceLROnPlateau.
    """)
    return
...
def _(jax, jnp):
    ...
    return ReduceLROnPlateau, adam_init, adam_step


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ### 1.3 Code: Training Objective and Step

    The next cell builds the full training loop with:

    $$\mathcal{L}=\mathcal{L}_{\mathrm{data}}+\lambda\mathcal{R},\qquad
    \theta\leftarrow\theta-\alpha\nabla_\theta\mathcal{L},$$

    evaluated via `jax.value_and_grad`, with optional `jax.jit`.
    """)
    return
...
def _(mo):
    ...
    **Tuning sequence:**
    1. Toggle **Use regularization term** on and start at `reg_strength = 0.01`.
    2. Increase strength if dynamics collapse to near-constant trajectories.
    3. If optimization becomes unstable, reduce `lr` and turn on clipping.
    4. To mimic tuned behavior, also enable normalization, He init, weight decay, and scheduler.
    """)
    return
