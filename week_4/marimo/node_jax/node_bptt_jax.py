...
def _(mo):
    ...
    and train a Neural ODE model

    $$\frac{dx}{dt}=f_\theta(x),\qquad x(t_0)=x_0.$$

    On a grid $\{t_k\}_{k=0}^{N}$ with $h_k=t_{k+1}-t_k$, RK4 gives

    $$x_{k+1}\approx \Phi_\theta(x_k,h_k).$$
    ...
    $$\mathcal{L}(\theta)=\frac{1}{MN}\sum_{i=1}^{M}\sum_{k=0}^{N}
    \|x_k^i-\hat{x}_k^i\|_2^2+\lambda R(\theta).$$
    ...
    BPTT differentiates through all solver steps. If
    $s_k=\partial\mathcal{L}/\partial x_k$, then

    $$s_k=s_{k+1}\frac{\partial\Phi_\theta}{\partial x_k}
    ...
    - $x(t)\in\mathbb{R}^n$: continuous state, $x_k\approx x(t_k)$.
    - $x_0$: initial state.
    - $f_\theta$: neural vector field with parameters $\theta$.
    - $M$: number of trajectories, $N$: number of time steps.
    - $\hat{x}_k^i$: observed state for trajectory $i$ at index $k$.

    JAX computes gradients with `jax.value_and_grad`, and optionally accelerates
    updates with `jax.jit`.
    """)
    return
...
def _(mo):
    ...
    The learnable dynamics is a two-hidden-layer MLP:

    $$f_\theta(x)=W_3\,\rho\!\left(W_2\,\rho(W_1x+b_1)+b_2\right)+b_3,$$

    where $\rho$ is ReLU. The notebook lets you switch between:

    - vanilla Xavier-like initialization (baseline),
    - He initialization (often more stable for ReLU networks).
    """)
    return
...
def _(mo):
    ...
    In plain language:
    Adam adapts step sizes per parameter, clipping limits exploding updates,
    and the scheduler reduces learning rate when MSE stops improving.
    """)
    return
...
def _(mo):
    ...
    1. forward trajectory simulation on all $M$ samples,
    2. loss evaluation ($\mathcal{L}_{\text{data}}$ and optional $\lambda R$),
    3. gradient computation through all time steps (BPTT),
    4. parameter update with Adam.
...
def _(mo):
    ...
    **Tuning sequence:**
    ...
    Practical note:
    - larger `N` gives finer time resolution but higher cost,
    - larger `M` improves coverage of initial states but also increases compute.
    """)
    return
