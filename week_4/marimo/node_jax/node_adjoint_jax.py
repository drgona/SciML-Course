...
def _(mo):
    mo.md(r"""
    ---
    ## 1. RK4 NODE and Custom Adjoint-VJP Formulation

    Dynamics and learned vector field:
    ...
    Discrete integration map:

    $$y=\operatorname{Integrate}_\theta(x_0,t_0,\dots,t_N).$$

    Forward pass stores $(\theta, t, y)$.

    In backward pass, for each reverse step we apply a VJP of one RK4 step
    and accumulate parameter sensitivities:

    $$a_{k-1} = g_{k-1} + \left(\frac{\partial \Phi_\theta}{\partial x_{k-1}}\right)^\top a_k,$$

    $$\nabla_\theta \mathcal{L} = \sum_{k=1}^{N}
    \left(\frac{\partial \Phi_\theta}{\partial \theta}\right)^\top a_k.$$

    This mirrors adjoint-style reverse accumulation while staying fully in JAX via `custom_vjp`.
    """)
    return
...
def _(jax, jnp):
    ...
    return pendulum_rhs, rk4_integrate, rk4_step, sample_ic
...
def _(mo):
    ...
    and a custom backward rule that scans backward in time and accumulates
...
@app.cell(hide_code=True)
def _(jax, jnp, mlp_apply, rk4_integrate, rk4_step, tree_add, tree_zeros_like):
...
def _(mo):
    ...
    via `jax.value_and_grad`, with optional JIT compilation.
    """)
    return
